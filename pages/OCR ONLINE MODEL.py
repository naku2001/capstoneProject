...
from functions import convert_pdf_to_txt_pages,login,convert_pdf_to_txt_file, save_pages, displayPDF, images_to_txt
...
st.markdown("""
    ## Text data extractor: Image to Text
...
with st.sidebar:
    st.title(":outbox_tray: Extract Text")
    ...
    ocr_box_2 = st.checkbox('Enable OCR(handwritten document)')
    ...
    st.markdown(html_temp.format("rgba(55, 53, 47, 0.16)"),unsafe_allow_html=True)
    st.markdown("""
    Made by [@perfect](https://twitter.com/nainia_ayoub) 
    """)
    st.markdown(
        """
...
if pdf_file:
    ...
    if file_extension == "pdf":
        ...
        text = result.text  
        # text2 = detect_handwriting(pdf_file)
